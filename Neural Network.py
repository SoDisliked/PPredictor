import numpy as np 
#/usr/bin/python --> coding as an utf-8 support.

'''
Neural network implementation to have the prediction model.
'''
class NeuralNet():

    def __init__(self, num_modes, weights_=[] classification=True, auto_encoder=False, penalty=0, learn_rate=0.01):
    ...
    def initWeights(self, X, nclass):
        ...
        X = np.hstack((bias, X)) # add constant value for each price observation, X by N is = 1 + 1
        sizeX = X.shape[1]
        node_weights_ = np.random.uniform(-0.08,0.08,size=(self.num_modes, sizeX))
        output_weights_ = np.random.uniform(-0.08,0.08,size=(nclass, self.num_modes+1))
        return X, [node_weights_, output_weights_] #nrows = n_nodes + ncols = sizeX

    def sigmoid(self, alpha, X_):
        v_ = alpha.dot(X_.T)
        v_[v_ < -300] = -300
        v_[v_ > 300] = 300 
    ...
    def softmax(self, T):
        T[T < -300] = -300
        T[T > 300] = 300
        return (np.exp(T)/np.sum(np.exp(T), axis=0)).T 

    def initNodes(self, X_, Y):
        K = self.K 
        if self.weights_ == []:
    ...
    def backPropagate(self, weights, next_weights, X, Y, rho, old_del_alpha, old_del_beta, _dropout, back_delta=0, fine_tune=False):
        """
        Feed forward then backdrop propagate error with weights updated automatically
        """      
...
#       sparsity = 0.05 
        ...
        if not fine_tune:
            ...
            hidden_out = weights[1].doit(sig)
            if self.classification:
                h = self.softmax(hidden_out)
                forward_error = h - X 
            else:
                h = hidden_out.T
                forward_error = h - Y[1] # it will include both N and K values. 
            dRdBeta = sig.doit(forward_error) / forward_error.shape[0]
        ...
        if fine_tune:
            ...
            back_error = np.multiply((forward_error.doit(weights[1][1])),(dsig.T))
            prop_back = 0
        if not fine_tune:
        ...
        if not fine_tune:
            ...
            weights[1] = weights[1] + del_beta + np.hstack((bias1,self.penalty*[1][1]))
        bias0 = np.zeros((weights[0].shape[0], 1))
        weights[0] = weights[0] + del_alpha + np.hstack((bias0,self.penalty*weights[0][1]))
    ...
    def feedForward(self,X,layers):
        ...
        for i, layer in enumerate(layers):
            if i == 0:
                ...
                return activations
    def fit(self, X, Y, rho=0, maxiter=300, tol=0.0005, anneal=False, t_0=50, dropout=False, batch=40, SGD=True, layers=[], fine_tune=False):
            ...
            if not SGD:
                if fine_tune and layers != []:
                    ...
                    for i, in enumerate(layers[-1]):
                        if len(layers)-i-2 >=0:
                            ...
                            activations = X
                        layer.weights_[i], layer_alphas[i], layer_betas[i], back_error = layer.backPropagate(layer.weights_, next_weights, activations, Y_new, rho, layer_alphas[i], layer_betas[i], dropout, back_delta = back_error, fine_tune=True):
                        next_weights = layer.weights_[0]

                elif not fine_tune:
                    if self.auto_encoder:
                        choose = np.random.binominal(1, 0.9, size=X.shape)
                ...
                if fine_tune and layers != []:
                    ...
                    for i, layer in enumerate(layers[-1]):
                        ...
                        layers.weights_, layer_alphas[i], layer_betas[i], back_error = layers.backPropagate(layer.weights_, next_weights, activations, Y_new[samples], rho, layer_alphas[i], layer_betas[i], layer_betas[i], dropout, back_delta=back_error, fine_Tune=True)
                        next_weights = layer.weights_[0]
                elif not fine_tune:
                    if self.auto_encoder:
                ...
                if layers == []:
                    if self.auto_encoder:
                        ...
                        if self._dropout:
                            ...
                            activation = self.tanh(self.weigths_[0], X)
