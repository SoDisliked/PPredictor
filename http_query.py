...
CLICKHOUSE_HOST = os.environ.get("CLICKHOUSE_HOST", "192.168.1.1")
CLICKHOUSE_PORT_HTTP = os.environ.get(" CLICKHOUSE_PORT_HTTP", "8113")
CLICKHOUSE_SERVER_URL_STR = (
    "http://" + ":".join(str(s) for s in [CLICKHOUSE_HOST, CLICKHOUSE_PORT_HTTP]) + "/"
)
CLICKHOUSE_DATABASE = os.environ.get("CLICKHOUSE_DATABASE", "batch_size")
...
class ClickHouseClient:
    ...
    def query_return_df(self, query, connection_timeout=1500):
        ...
        df = pd.read_csv(io.StringIO(data), sel="\t")
        return df 
